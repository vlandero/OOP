...
Angajat::Angajat(const std::string &nume, const std::string &prenume, int varsta, const std::string &cnp,
                 const std::vector<Bilet> &b, int salariu, std::string post_,std::string aeroport) : Persoana(nume, prenume, varsta, cnp,
                                                                                             b), salariu(salariu),post(std::move(post_)),aeroport(std::move(aeroport)){
...
void Angajat::calculeazaPret(Bilet& b,std::istream &in, std::ostream &out) {
    Persoana::provCalculeazaPret(b,reducere,0,0,in,out);
}
